...
/** Validates user-editable NEXUS settings before they reach runtime components. */
object NexusSettingsPolicy {
    ...
    fun validate(input: Settings): ValidationResult {
        val errors = buildList {
            ...
            if (input.networkToolsEnabled && !input.confirmationRequiredForConsequentialActions) {
                add("networkToolsEnabled requires consequential-action confirmation")
            }
        }
        return ValidationResult(input, errors)
    }

    fun sanitize(input: Settings): Settings = input.copy(
        wakeWordSensitivity = input.wakeWordSensitivity.coerceIn(0f, 1f),
        ragTopK = input.ragTopK.coerceIn(1, 50),
        maxAutomationSteps = input.maxAutomationSteps.coerceIn(1, 100)
    )
}
