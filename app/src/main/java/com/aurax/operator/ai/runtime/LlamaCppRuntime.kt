...
class LlamaCppRuntime(context: Context) : AIModelRuntime {
    ...
    companion object { private var loaded = runCatching { System.loadLibrary("aurax_native"); true }.getOrDefault(false) }
    ...
    suspend fun generateFrom(modelFile:File,request:GenerationRequest):String {
        check(loaded) { "Native local engine is unavailable" }; check(modelFile.isFile) { "Local model file is unavailable: ${modelFile.absolutePath}" }
        thermal.refresh()
        ...
        return nativeGenerate(modelFile.absolutePath,request.prompt,request.safeMaxTokens.coerceAtMost(output),request.safeTemperature,request.safeContextTokens.coerceAtMost(context),threads.coerceIn(1,12),batch.coerceIn(32,2048),settings.topK.coerceIn(1,200))
    }
