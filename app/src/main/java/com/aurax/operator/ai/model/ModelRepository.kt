    ...
    fun importPrimaryModel(uri: Uri): File {
        ...
        try {
            context.contentResolver.openInputStream(uri).use { input ->
                ...
                tmp.outputStream().use { output -> input.copyTo(output, bufferSize = COPY_BUFFER_SIZE) }
            }
...
data class ModelStatus(
    ...
    companion object {
        ...
        fun validateFile(file: File): ModelStatus {
            ...
            if (size <= MIN_MODEL_BYTES) return invalid(size, "Model file is too small")
            ...
            if (!magicOk) return invalid(size, "File is not a GGUF model")
            ...
            }.getOrElse { return invalid(size, "Unable to read model for integrity check") }
            return valid(size, digest.digest().joinToString("") { "%02x".format(it) })
        }
    }
}
