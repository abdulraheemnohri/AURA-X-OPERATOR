...
class ModelHub @Inject constructor(
    ...
    suspend fun importModel(file: File, metadata: ImportedModelMetadata): ModelEntity {
        ...
        val entity = ModelEntity(
            ...
            category = metadata.category, format = metadata.format, quantization = metadata.quantization,
            sourceUrl = "", localPath = destination.absolutePath, sizeBytes = destination.length(),
            downloadedBytes = destination.length(), sha256 = hash, contextLength = metadata.contextLength,
            parameters = metadata.parameters, status = "READY", isUserImported = true,
            importDate = System.currentTimeMillis(), tags = metadata.tags, description = metadata.description,
