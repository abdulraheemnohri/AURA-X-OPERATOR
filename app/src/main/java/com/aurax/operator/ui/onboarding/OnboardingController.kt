...
class OnboardingController(context: Context) {
    ...
    var completed: Boolean
        ...
        private set(value) { prefs.edit().putBoolean("completed", value).apply() }
    var step: Step
        get() = runCatching { Step.valueOf(prefs.getString("step", Step.WELCOME.name) ?: Step.WELCOME.name) }.getOrDefault(Step.WELCOME)
        private set(value) { prefs.edit().putString("step", value.name).apply() }
    fun advance() {
        step = when (step) {
